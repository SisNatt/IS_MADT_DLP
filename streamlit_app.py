...
# Focus on Match_Label = False
elif selected == "Incident Analysis":
    st.title("📉 Focus on False - Analysis")
    ...
    if 'processed_file' in st.session_state:
        try:
            ...
            # Ensure Match_Label is valid
            df_processed['Match_Label'] = df_processed['Match_Label'].apply(
                lambda x: True if str(x).strip().lower() == 'true' else False
            )

            # Filter data with Match_Label = False
